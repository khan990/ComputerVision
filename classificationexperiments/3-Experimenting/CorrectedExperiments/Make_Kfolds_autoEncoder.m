...
function [Tr_features_ret, Te_features, Tr_Labels_ret, Te_Labels_ret] = Make_Kfolds_autoEncoder(features, Labels, Num_of_folds)
...
%         Tr_features_ret{i} = zeros(size(features, 1), chunks);
        Tr_features_ret{i} = cell(size(features, 1), chunks);
        Tr_Labels_ret{i} = zeros(Num_of_Classes, chunks);
        
%         Te_features{i} = zeros(size(features, 1), chunks);
        Te_features{i} = cell(size(features, 1), chunks);
